...
# EXACT LATITUDE & LONGITUDE FOR EVERY EEZ & COUNTRY (NO FALLBACK OCEAN SCATTER)
EEZ_COORDINATES = {
    # --- EUROPE (ATLANTIC & BAY OF BISCAY) ---
    ...
    # --- EUROPE (NORTH SEA, BALTIC & NORDICS) ---
    ...
    "bulgarian": [43.2100, 27.9100],  # BULGARIA -> REAL BLACK SEA COAST (43.21 N, 27.91 E)
    ...
    # --- MIDDLE EAST & AFRICA ---
    "eritrean": [15.1700, 39.7800],  # ERITREA -> RED SEA COAST
    "eritrea": [15.1700, 39.7800],
...
def get_eez_coordinates(clean_title, filename):
    lookup_str = f"{clean_title} {filename}".lower()

    for key, coords in EEZ_COORDINATES.items():
        if key in lookup_str:
            # Deterministic small offset for multiple files of the same nation
    ...
    # Fallback near Lisbon if entirely unrecognized
...
def process_all_config_csvs():
    ...
    for file_idx, f in enumerate(csv_files):
        ...
        coords = get_eez_coordinates(display_title, file_base)
    ...
    print(f"SUCCESS: Exported {len(final_ports)} distinct EEZ port records.")
